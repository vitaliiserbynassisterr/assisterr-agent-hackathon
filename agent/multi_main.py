...
async def network_overview():
    ...
    for st in all_states:
        ...
        # Use running counters (not capped list lengths)
        total_interactions += st.counter_a2a
        total_on_chain += st.counter_on_chain + n_merkle_batches
        total_evaluations += st.counter_evaluations
        total_activities += st.counter_activities
        total_economic_txs += st.counter_economic_txs
        total_adaptive += st.counter_adaptive
        ...
        agent_summaries.append({
            ...
            "a2a_interactions": st.counter_a2a,
            "on_chain_txs": st.counter_on_chain + n_merkle_batches,
    ...
    total_merkle_batches = sum(
        st.audit_batcher.total_batches_stored if st.audit_batcher else 0
        for st in all_states
    )
    total_audit_entries = sum(
        st.audit_batcher.total_entries_logged if st.audit_batcher else 0
        for st in all_states
    )
